import { createStripeCustomer, updateStripeCustomer } from '../../server/services/stripe';
import { User, UserModel } from '../models/User';

export async function createUser(fullName: string, phoneNumber: string, email: string) {
  try {
...
export async function getUserByPhoneNumber(phoneNumber: string) {
  try {
    ...
    throw new Error(`Error getting user by phone number: ${error.message}`);
  }
}
